...
\begin{figure}[htbp]
    ...
    \scalebox{1.0}{
      ...
      \begin{tikzpicture}[x=1in,y=1in]
        ...
        \node [Assembly, fill=\lowenr] at ($(-3*\latWidth,3*\latWidth)$) {$\mathrm{A}$}; % L5
        ...
        \node [Assembly, fill=\lowenr] at ($( 3*\latWidth,3*\latWidth)$) {$\mathrm{A}$}; % E5
        ...
        \node [Assembly, fill=\lowenr] at ($(-3*\latWidth,-3*\latWidth)$) {$\mathrm{A}$}; % L11
        ...
        \node [Assembly, fill=\lowenr] at ($( 3*\latWidth,-3*\latWidth)$) {$\mathrm{A}$}; % E11
